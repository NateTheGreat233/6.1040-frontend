...
export default class DualProfileConcept {
  ...
  public async deleteFromScrapbook(user: ObjectId, scrapbookId: string) {
    ...
    const scrapbook = profile.scrapbook;
    const newScrapbook = scrapbook.filter((scrapbook) => scrapbook.id !== scrapbookId);
    await this.dualProfiles.updateOne({ $or: [{ user1: user }, { user2: user }] }, { scrapbook: newScrapbook });
  }
  ...
  public async addToScrapbook(user: ObjectId, entry: { image: { buffer: Uint8Array; mimeType: string }; caption: string; date: Date }) {
    ...
    const updateResult = await this.dualProfiles.updateOne(
      ...
      { scrapbook: [...existingScrapbook, { imageUrl, caption: entry.caption, date: entry.date, id: uuidv4() }] },
    );
    if (updateResult.modifiedCount !== 1) {
      throw new NotFoundError("Could not find a dual profile");
    }
